...
- HTML parsing and sanitization load atomically. Footnote IDs stay in the document namespace; the only allowed input is an enforced disabled checkbox for task lists. Mermaid initialization and rendering are serialized, chart fences use the strict JSON `version: 1` / `renderer: "echarts"` envelope, and diagram styling is independent of the HTML article-formatting setting.
- Dedicated highlight, math, diagram, chart, and HTML styles load with their extension modules. `chart` is the standard ECharts fence name; `markdown-chart` remains a compatibility alias. GFM and HTML reuse the table stylesheet. Global CSS retains semantic tokens, CommonMark typography, shared attachment/code shells, and export/print overrides.
